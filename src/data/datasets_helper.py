...
def filter_labels(X, y, label_a, label_b):
    ...
    return X[indices], [l[indices] for l in y]


def split_datasets(X, y, perc=0.3, seed=42):
    assert all([len(l) == len(X) for l in y])

    indices = range(len(X))
    indices_train, indices_test = train_test_split(indices, test_size=perc, random_state=seed)
