...
def holdouts_experiments_executor(exp_name, X, y, logger, path_logs, model_fun, input_type):
    ...
    for t in tasks_dict:
        ...
        for h in range(holdouts):
            ...
            tuner, _, best_hyperparams = hp_tuner(X_train_int if input_type == "epi" else X_train_int[0],
                                                  y_train_int,
                                                  X_val if input_type == "epi" else X_val[0],
                                                  y_val,
                                                  model_fun,
                                                  exp_name,
                                                  weight_class,
            ...
            best_model, history = model_trainer(X_train if input_type == "epi" else X_train[0],
                                                y_train,
                                                X_test if input_type == "epi" else X_test[0],
                                                y_test,
                                                model_fun,
                                                exp_name,
                                                weight_class,
